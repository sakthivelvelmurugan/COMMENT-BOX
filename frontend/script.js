...
async function generateComments() {
  ...
  const apiKey = apiKeyInput.value.trim() || localStorage.getItem('cb-apikey');
  if (!apiKey) { showToast('Enter your OpenRouter API key below', true); return; }

  setLoading(true);
  clearOutput();

  const prompt = buildPrompt(currentLang, code, commentStyle.value, density.value);
